                                ...
                                <tbody id="searchTable">
                                    @foreach ($documents2 as $temp)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $trainingPlanName->training_end_date ? \Carbon\Carbon::parse($trainingPlanName->training_end_date)->format('d M Y H:i A') : '-' }}
                                            </td>
                                            <td>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @foreach ($employees->sortbyDesc('id') as $employee)
                                <tr>
                                    <td><a
                                            href="{{ url('employee_view', $employee->id) }}">{{ $employee->employee_id }}</a>
                                    </td>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($trainers->sortbyDesc('id') as $trainer)
                                    <tr>
                                        <td><a
                                                href="{{ url('trainer_qualification_view', $trainer->id) }}">000{{ $trainer->id }}</a>
                                        </td>
                                        <td>{{ $trainer->trainer_name ? $trainer->trainer_name : 'NA' }}</td>
                                        <td>{{ $trainer->department_record ? $trainer->department_record->name : 'NA' }}
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($employee->certification)
                                                <a href="{{ asset('upload/' . $employee->certification) }}" target="_blank"
                                                    download>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($jobTrainings as $job_training)
                                    <tr>
                                        <td>{{ DB::table('employees')->where('id', $job_training->name)->value('employee_name') }}
                                        </td>
                                        <td>{{ DB::table('departments')->where('id', $job_training->department)->value('name') }}
                                        </td>
                                ...
                                <tr class="training-row" data-status="{{ $training->status == 'Complete' ? 'complete' : ($training->training_end_date < now() ? 'past_due_date' : 'pending') }}">
                                        ...
                                        @php
                                            ...
                                            foreach ($sopIds as $sopId) {
                                                ...
                                                if ($documentDetails) {
                                                    $divisionName = Helpers::getDivisionName(
                                                        $documentDetails->division_id,
                                                    );
                                                    $recordFormat = Helpers::recordFormat($documentDetails->record);
                                                    $formattedRecord =
                                                        $divisionName . '/Document/' . date('Y') . '/' . $recordFormat;
                                        ...
                                        <td style="justify-items: center">{{ $completionDate }}</td>
                                    </tr>
                                @endforeach
