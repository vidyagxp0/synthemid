...
class TMSController extends Controller
{
    ...
    public function logsReportTranee($trainingId, $traineeId)
{
    $training = Training::find($trainingId);
    
    $user_trainee_id=$traineeId;
    
    $document = Document::where('id', $training->sops)->latest()->first();
    $trainingstatus = TrainingStatus::where(['user_id' => $traineeId, 'training_id' => $trainingId])->latest()->first();


    $pdf = PDF::loadView('frontend.TMS.pdfpagetranee', compact('training', 'document','trainingstatus','user_trainee_id'));

    return $pdf->stream('training_report' . '.pdf');
}
